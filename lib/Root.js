...
const consoleTheme = atom.config.get('core.themes')[0].match(/(light|dark)/)[0] || 'dark'
...
export default class Root extends React.Component {
  ...
  componentDidMount() {
    ...
    this.disposeListener = listen(({ type, ...data }) => {
      ...
      switch (type) {
        ...
        case 'urlchange':
          this.setState(({ locations, currentLocationIndex, weDidInitiateNavigation }) => {
  ...
  render() {
    const { logs, locations, currentLocationIndex, showConsole, status } = this.state
    ...
    return (
      <div className="codesandbox">
        <div className="toolbar" style={{ justifyContent: 'flex-end' }}>
          <div className="text-subtle" style={{ flexGrow: 1, padding: '0 10px' }}>
            {status}
          </div>
          ...
          >
            <div className="preview" style={{ marginBottom: showConsole && 0 }}>
              <div className="toolbar">
                ...
                <input type="text" disabled value={location} className="input-text" />
              </div>
